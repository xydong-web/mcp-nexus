...
export function KeysPage({ api }: { api: AdminApi }) {
  ...
  // Grok keys state
  const [grokKeys, setGrokKeys] = useState<GrokKeyDto[]>([]);
  ...
  // Grok key dialog state
  const [grokCreateOpen, setGrokCreateOpen] = useState(false);
  const [grokCreating, setGrokCreating] = useState(false);
  const [newGrokLabel, setNewGrokLabel] = useState('');
  const [newGrokApiKey, setNewGrokApiKey] = useState('');
  const [grokTouched, setGrokTouched] = useState<{ label?: boolean; apiKey?: boolean }>({});
  const [grokKeyToDelete, setGrokKeyToDelete] = useState<GrokKeyDto | null>(null);
  ...
  const loadGrokKeys = useCallback(async () => {
    setGrokLoading(true);
    setGrokError(null);
    try {
      setGrokKeys(await api.listGrokKeys());
    } catch (e: any) {
      setGrokError(typeof e?.message === 'string' ? e.message : tc('errors.unknownError'));
      setGrokKeys([]);
    } finally {
  ...
  useEffect(() => {
    ...
    void loadBraveKeys();
    void loadGrokKeys();
  }, [load, loadBraveKeys, loadGrokKeys]);
  ...
  const grokFormErrors = useMemo(() => {
    const errors: { label?: string; apiKey?: string } = {};
    if (grokTouched.label && !newGrokLabel.trim()) {
      errors.label = t('form.labelRequired');
    } else if (grokTouched.label && newGrokLabel.length < 2) {
      errors.label = t('form.labelMinLength');
    }
    if (grokTouched.apiKey && !newGrokApiKey.trim()) {
      errors.apiKey = t('form.apiKeyRequired');
    }
    return errors;
  }, [grokTouched, newGrokApiKey, newGrokLabel, t]);

  const isGrokFormValid = !grokFormErrors.label && !grokFormErrors.apiKey && newGrokLabel.trim() && newGrokApiKey.trim();
  ...
  async function onCreateGrokKey() {
    if (!newGrokLabel.trim() || !newGrokApiKey.trim()) return;
    setGrokCreating(true);
    try {
      await api.createGrokKey({ label: newGrokLabel.trim(), apiKey: newGrokApiKey.trim() });
      toast.push({ title: t('toast.added'), message: t('toast.addedMessage', { name: newGrokLabel.trim() }), variant: 'success' });
      setGrokCreateOpen(false);
      setNewGrokLabel('');
      setNewGrokApiKey('');
      setGrokTouched({});
      await loadGrokKeys();
      ...
      setGrokCreating(false);
    }
  }

  async function onUpdateGrokStatus(id: string, status: GrokKeyStatus) {
    try {
      await api.updateGrokKeyStatus(id, status);
      toast.push({ title: t('toast.updated'), message: t('toast.updatedMessage', { status }), variant: 'success' });
      await loadGrokKeys();
    } catch (e: any) {
      toast.push({ title: t('toast.updateFailed'), message: typeof e?.message === 'string' ? e.message : tc('errors.unknownError'), variant: 'error' });
  ...
  async function onDeleteGrokKey() {
    if (!grokKeyToDelete) return;
    setGrokDeleting(true);
    try {
      await api.deleteGrokKey(grokKeyToDelete.id);
      toast.push({ title: t('toast.deleted'), message: t('toast.deletedMessage', { name: grokKeyToDelete.label }), variant: 'success' });
      setGrokKeyToDelete(null);
      await loadGrokKeys();
    } catch (e: any) {
      toast.push({ title: t('toast.deleteFailed'), message: typeof e?.message === 'string' ? e.message : tc('errors.unknownError'), variant: 'error' });
    } finally {
  ...
  const grokStats = useMemo(() => {
    const active = grokKeys.filter((k) => k.status === 'active').length;
    const disabled = grokKeys.filter((k) => k.status === 'disabled').length;
    const cooldown = grokKeys.filter((k) => k.status === 'cooldown').length;
    const invalid = grokKeys.filter((k) => k.status === 'invalid').length;
    const total = grokKeys.length;
    const activeRate = total > 0 ? Math.round((active / total) * 100) : 0;
    return { active, disabled, cooldown, invalid, total, activeRate };
  }, [grokKeys]);
  ...
  return (
    <div className="stack">
        ...
        <>
          ...
          <div className="card">
            <div className="cardHeader">
              <div className="row">
                <div>
                  <div className="h2">Grok keys</div>
                  <div className="help">
                    {grokStats.total} total, {grokStats.active} active, {grokStats.cooldown} cooldown, {grokStats.invalid} invalid, {grokStats.disabled} disabled
                  </div>
                </div>
                <div className="flex gap-3 items-center">
                  <button className="btn" onClick={loadGrokKeys} disabled={grokLoading}>
                    <IconRefresh className={grokLoading ? 'spin' : ''} />
                    {t('actions.refresh')}
                  </button>
                  <button className="btn" data-variant="primary" onClick={() => setGrokCreateOpen(true)}>
                    <IconPlus />
                    Add Grok key
                  </button>
                </div>
            ...
            <div className="cardBody p-0">
              {grokError ? (
                <div className="p-4">
                  <ErrorBanner message={grokError} onRetry={loadGrokKeys} retrying={grokLoading} />
                </div>
              ) : null}
                    ...
                    },
                    {
                      ...
                      cell: (k: GrokKeyDto) => (
                        <KeyRevealCell
                          keyId={k.id}
                          maskedKey={k.maskedKey || '************'}
                          api={{ revealKey: api.revealGrokKey } as any}
                        />
                      )
                    },
                    {
                      ...
                      cell: (k: GrokKeyDto) => (
                        <StatusMenu
                          status={k.status}
                          options={['active', 'disabled', 'cooldown', 'invalid']}
                          onChange={(s) => onUpdateGrokStatus(k.id, s as GrokKeyStatus)}
                        />
                      )
                    },
                    {
                      ...
                      dataLabel: t('table.lastUsed'),
                      cellClassName: 'mono',
                      cell: (k: GrokKeyDto) => formatDateTime(k.lastUsedAt)
                    },
                    {
                      id: 'created',
                      header: t('table.created'),
                      ...
                      dataLabel: t('table.created'),
                      cellClassName: 'mono',
                      cell: (k: GrokKeyDto) => formatDateTime(k.createdAt)
                    },
                    {
                      id: 'actions',
                      header: '',
                      headerStyle: { width: '4%', textAlign: 'right' },
                      headerAlign: 'right',
                      dataLabel: t('table.actions'),
                      cellAlign: 'right',
                      cell: (k: GrokKeyDto) => (
                        <IconButton
                          icon={<IconTrash />}
                          variant="ghost-danger"
                          onClick={() => setGrokKeyToDelete(k)}
                          title="Delete Grok key"
                          aria-label="Delete Grok key"
                        />
                      )
                    }
                  ] satisfies DataTableColumn<GrokKeyDto>[]
                )}
                rows={grokKeys}
                rowKey={(k) => k.id}
                loading={grokLoading && grokKeys.length === 0}
                empty={
                  <EmptyState
                    icon={<IconKey />}
                    message="No Grok keys configured yet."
                    action={{ label: 'Add Grok key', onClick: () => setGrokCreateOpen(true) }}
                  />
                }
              />
            </div>
      ...
      <Dialog title="Add Grok key" open={grokCreateOpen} onClose={() => (grokCreating ? null : setGrokCreateOpen(false))}>
        <div className="stack">
          <div className="grid2">
            <div className="stack">
              <label className="label" htmlFor="grok-key-label">
                {t('form.label')} <span style={{ color: 'var(--danger)' }}>{t('form.required')}</span>
              </label>
              <input
                id="grok-key-label"
                className="input"
                data-error={!!(grokFormErrors.label && grokTouched.label)}
                value={newGrokLabel}
                onChange={(e) => setNewGrokLabel(e.target.value)}
                onBlur={() => setGrokTouched((prev) => ({ ...prev, label: true }))}
                placeholder={t('form.labelPlaceholder')}
                autoComplete="off"
                aria-invalid={!!(grokFormErrors.label && grokTouched.label)}
                aria-describedby={grokFormErrors.label && grokTouched.label ? 'grok-label-error' : 'grok-label-help'}
              />
              {grokFormErrors.label && grokTouched.label ? (
                <div id="grok-label-error" className="fieldError">
                  {grokFormErrors.label}
                </div>
              ) : (
                <div id="grok-label-help" className="help">
                  {t('form.labelHelp')}
                </div>
              )}
            </div>
            <div className="stack">
              <label className="label" htmlFor="grok-api-key">
                {t('form.apiKey')} <span style={{ color: 'var(--danger)' }}>{t('form.required')}</span>
              </label>
              <input
                id="grok-api-key"
                className="input mono"
                data-error={!!(grokFormErrors.apiKey && grokTouched.apiKey)}
                value={newGrokApiKey}
                onChange={(e) => setNewGrokApiKey(e.target.value)}
                onBlur={() => setGrokTouched((prev) => ({ ...prev, apiKey: true }))}
                placeholder="xai-..."
                autoComplete="off"
                aria-invalid={!!(grokFormErrors.apiKey && grokTouched.apiKey)}
                aria-describedby={grokFormErrors.apiKey && grokTouched.apiKey ? 'grok-apikey-error' : 'grok-apikey-help'}
              />
              {grokFormErrors.apiKey && grokTouched.apiKey ? (
                <div id="grok-apikey-error" className="fieldError">
                  {grokFormErrors.apiKey}
                </div>
              ) : (
                <div id="grok-apikey-help" className="help">
                  Enter a Grok API key.
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            <button className="btn" onClick={() => setGrokCreateOpen(false)} disabled={grokCreating}>
              {tc('actions.cancel')}
            </button>
            <button className="btn" data-variant="primary" onClick={onCreateGrokKey} disabled={grokCreating || !isGrokFormValid}>
              <IconKey />
              {grokCreating ? t('button.adding') : 'Add Grok key'}
            </button>
          </div>
        </div>
      </Dialog>
      ...
      <ConfirmDialog
        open={!!grokKeyToDelete}
        title={t('dialog.deleteTitle')}
        description={t('dialog.deleteDescription', { label: grokKeyToDelete?.label ?? '' })}
        confirmLabel={tc('actions.delete')}
        confirmVariant="danger"
        requireText="DELETE"
        requireTextLabel={t('dialog.requireDeleteText')}
        confirming={grokDeleting}
        onClose={() => (grokDeleting ? null : setGrokKeyToDelete(null))}
        onConfirm={onDeleteGrokKey}
      />
