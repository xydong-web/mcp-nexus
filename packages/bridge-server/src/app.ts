...
import { GrokKeyPool } from './grok/keyPool.js';
import { RotatingGrokClient } from './grok/rotatingClient.js';
...
export function createBridgeApp(options: CreateBridgeAppOptions = {}): express.Express {
  ...
  const grokPool = new GrokKeyPool({
    prisma,
    encryptionKey,
    getSelectionStrategy: () => serverSettings.getGrokKeySelectionStrategy()
  });
  const grokClient = new RotatingGrokClient({
    pool: grokPool,
    prisma,
    tavilyClient,
    braveClient,
  ...
  app.post('/mcp', asyncHandler(async (req: any, res: any) => {
    ...
    if (hasGrokToolsCallRequest(req.body) && await serverSettings.getGrokSearchEnabled()) {
      const check = await grokClient.preflightEligible();
      if (!check.ok) {
        ...
        try {
          ...
          if (!sessionId && isInitializeRequest(req.body)) {
            ...
            const server = createCombinedProxyServer({
              ...
              getGrokSourceMode: () => serverSettings.getGrokSearchSourceMode(),
              getResearchEnabled: () => serverSettings.getResearchEnabled(),
              getGrokSearchEnabled: () => serverSettings.getGrokSearchEnabled(),
              getGrokModelDefault: () => serverSettings.getGrokModelDefault(),
              getGrokExtraSourcesDefault: () => serverSettings.getGrokExtraSourcesDefault(),
              getAllowedTools: () => {
  ...
  app.get('/health', asyncHandler(async (_req, res) => {
    ...
    const activeBrave = await prisma.braveKey.count({ where: { status: 'active' } });
    const activeGrok = await prisma.grokKey.count({ where: { status: 'active' } });
    res.json({ ok: true, activeKeys: active, activeBraveKeys: activeBrave, activeGrokKeys: activeGrok });
  }));
...
export function hasGrokToolsCallRequest(body: unknown): boolean {
  if (!body) return false;
  if (Array.isArray(body)) return body.some(hasGrokToolsCallRequest);
  if (!isObjectRecord(body) || body.method !== 'tools/call') return false;

  const params = isObjectRecord(body.params) ? body.params : undefined;
  const toolName = params?.name;
  return typeof toolName === 'string' && ['web_search', 'get_sources', 'web_fetch', 'web_map'].includes(toolName);
}
