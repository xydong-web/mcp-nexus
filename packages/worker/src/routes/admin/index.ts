...
import { D1Client, generateId, type TavilyKey, type BraveKey, type GrokKey, type ClientToken } from '../../db/d1.js';
...
function parseSelectionStrategy(value: unknown, fallback: 'round_robin' | 'random'): 'round_robin' | 'random' {
...
function buildSettingsMap(settings: Array<{ key: string; value: string }>): Record<string, string> {
...
function buildServerInfo(
  ...
  opts: { activeBraveKeys: number; activeGrokKeys: number }
) {
  const tavilyKeySelectionStrategy = parseSelectionStrategy(
    settingsMap.tavilyKeySelectionStrategy ?? c.env.TAVILY_KEY_SELECTION_STRATEGY,
    'round_robin'
  );
  const searchSourceMode = parseSearchSourceMode(
    settingsMap.searchSourceMode ?? c.env.SEARCH_SOURCE_MODE,
    'brave_prefer_tavily_fallback'
  );
  const researchEnabled = parseBooleanSetting(
    settingsMap.researchEnabled,
    c.env.TAVILY_RESEARCH_ENABLED !== 'false'
  );
  const grokSearchEnabled = parseBooleanSetting(
  ...
  const grokSearchSourceMode = parseSearchSourceMode(
    settingsMap.grokSearchSourceMode
      ?? settingsMap.searchSourceMode
      ?? c.env.GROK_SEARCH_SOURCE_MODE
      ?? c.env.SEARCH_SOURCE_MODE,
    'combined'
  );
  const grokKeySelectionStrategy = parseSelectionStrategy(
    settingsMap.grokKeySelectionStrategy ?? c.env.GROK_KEY_SELECTION_STRATEGY,
    'round_robin'
  );
  ...
  return {
    ...
    searchSourceMode,
    braveSearchEnabled: opts.activeBraveKeys > 0,
    researchEnabled,
    grokSearchEnabled,
    grokModelDefault,
    ...
    grokActiveKeyCount: opts.activeGrokKeys
  };
}
...
adminRouter.get('/server-info', async (c) => {
  ...
  const [settings, braveKeys, grokKeys] = await Promise.all([
    db.getServerSettings(),
    db.getBraveKeys(),
    db.getGrokKeys()
  ]);
  const settingsMap = buildSettingsMap(settings);
  const activeBraveKeys = braveKeys.filter((key) => key.status === 'active').length;
  const activeGrokKeys = grokKeys.filter((key) => key.status === 'active').length;

  return c.json(
    buildServerInfo(c, settingsMap, {
      activeBraveKeys,
      activeGrokKeys
    })
  );
...
adminRouter.patch('/server-info', async (c) => {
  const body = await c.req.json<{
    ...
    grokModelDefault?: string;
    grokExtraSourcesDefault?: number;
    grokSearchSourceMode?: string;
    grokKeySelectionStrategy?: string;
  }>();
  const db = new D1Client(c.env.DB);
  const validModes = ['tavily_only', 'brave_only', 'combined', 'brave_prefer_tavily_fallback'];

  if (body.tavilyKeySelectionStrategy !== undefined) {
    if (body.tavilyKeySelectionStrategy !== 'round_robin' && body.tavilyKeySelectionStrategy !== 'random') {
      return c.json({ error: 'tavilyKeySelectionStrategy must be "round_robin" or "random"' }, 400);
    }
    await db.upsertServerSetting('tavilyKeySelectionStrategy', body.tavilyKeySelectionStrategy);
  }

  if (body.searchSourceMode !== undefined) {
    if (!validModes.includes(body.searchSourceMode)) {
      return c.json({ error: 'searchSourceMode must be one of: tavily_only, brave_only, combined, brave_prefer_tavily_fallback' }, 400);
    }
  ...
  if (body.researchEnabled !== undefined) {
    if (typeof body.researchEnabled !== 'boolean') {
      return c.json({ error: 'researchEnabled must be a boolean' }, 400);
    }
    await db.upsertServerSetting('researchEnabled', String(body.researchEnabled));
  }

  if (body.grokSearchEnabled !== undefined) {
    if (typeof body.grokSearchEnabled !== 'boolean') {
      return c.json({ error: 'grokSearchEnabled must be a boolean' }, 400);
    }
    await db.upsertServerSetting('grokSearchEnabled', String(body.grokSearchEnabled));
  }

  if (body.grokModelDefault !== undefined) {
    if (typeof body.grokModelDefault !== 'string' || !body.grokModelDefault.trim()) {
      return c.json({ error: 'grokModelDefault must be a non-empty string' }, 400);
    }
    await db.upsertServerSetting('grokModelDefault', body.grokModelDefault.trim());
  }

  if (body.grokExtraSourcesDefault !== undefined) {
    const value = Number(body.grokExtraSourcesDefault);
    if (!Number.isFinite(value) || value < 0 || value > 20) {
      return c.json({ error: 'grokExtraSourcesDefault must be a number between 0 and 20' }, 400);
    }
    await db.upsertServerSetting('grokExtraSourcesDefault', String(Math.floor(value)));
  }

  if (body.grokSearchSourceMode !== undefined) {
    if (!validModes.includes(body.grokSearchSourceMode)) {
      return c.json({ error: 'grokSearchSourceMode must be one of: tavily_only, brave_only, combined, brave_prefer_tavily_fallback' }, 400);
    }
    await db.upsertServerSetting('grokSearchSourceMode', body.grokSearchSourceMode);
  }

  if (body.grokKeySelectionStrategy !== undefined) {
    if (body.grokKeySelectionStrategy !== 'round_robin' && body.grokKeySelectionStrategy !== 'random') {
      return c.json({ error: 'grokKeySelectionStrategy must be "round_robin" or "random"' }, 400);
    }
    await db.upsertServerSetting('grokKeySelectionStrategy', body.grokKeySelectionStrategy);
  }
...
adminRouter.post('/tavily-keys', async (c) => {
  try {
    const body = await c.req.json<{ label: string; key?: string; apiKey?: string }>();
...
adminRouter.get('/grok-keys', async (c) => {
  const db = new D1Client(c.env.DB);
...
adminRouter.post('/grok-keys', async (c) => {
  ...
  try {
    ...
    await db.createAuditLog({
      eventType: 'grok_key.create',
      outcome: 'success',
      resourceType: 'grok_key',
      resourceId: id,
      ip,
      userAgent,
      detailsJson: JSON.stringify({ label })
    }).catch(() => {});

    return c.json({ id, label, keyMasked, status: 'active' }, 201);
  } catch (error: any) {
    const isUnique = typeof error?.message === 'string' && error.message.includes('UNIQUE constraint failed');
    await db.createAuditLog({
      eventType: 'grok_key.create',
      outcome: 'error',
      resourceType: 'grok_key',
      ...
      ip,
      userAgent,
      detailsJson: JSON.stringify({ error: error instanceof Error ? error.message : String(error) })
    }).catch(() => {});

    if (isUnique) {
      return c.json({ error: 'A key with this label already exists' }, 409);
    }
    return c.json({ error: 'Failed to create Grok key' }, 500);
  }
});

adminRouter.patch('/grok-keys/:id', async (c) => {
  const id = c.req.param('id');
  const ip = c.req.header('CF-Connecting-IP') ?? c.req.header('X-Forwarded-For') ?? null;
  const userAgent = c.req.header('User-Agent') ?? null;
  const db = new D1Client(c.env.DB);
  ...
  const existing = await db.getGrokKeyById(id);
  if (!existing) {
    await db.createAuditLog({
    ...
    return c.json({ error: 'Grok key not found' }, 404);
  }

  const updates: Partial<GrokKey> = {};
  ...
  if (body.status !== undefined) {
    ...
    if (!validStatuses.includes(body.status)) {
      return c.json({ error: 'status must be one of: active, disabled, cooldown, invalid' }, 400);
    }
    updates.status = body.status;
    if (body.status === 'cooldown') {
      if (body.cooldownUntil !== undefined && body.cooldownUntil !== null) {
        const parsed = Date.parse(body.cooldownUntil);
        if (!Number.isFinite(parsed)) {
          return c.json({ error: 'cooldownUntil must be a valid ISO datetime or null' }, 400);
        }
        updates.cooldownUntil = new Date(parsed).toISOString();
      } else {
        updates.cooldownUntil = new Date(Date.now() + 5 * 60_000).toISOString();
      }
    } else {
  ...
  if (body.key !== undefined) {
    ...
    if (!key) {
      return c.json({ error: 'key must be a non-empty string when provided' }, 400);
    }
    const encrypted = await encrypt(key, c.env.KEY_ENCRYPTION_SECRET);
    updates.keyEncrypted = encrypted.buffer as ArrayBuffer;
    updates.keyMasked = maskApiKey(key);
  }
  ...
  try {
    await db.updateGrokKey(id, updates);
    await db.createAuditLog({
      eventType: 'grok_key.update',
      outcome: 'success',
      resourceType: 'grok_key',
      ...
      detailsJson: JSON.stringify({
        ...
        keyRotated: updates.keyEncrypted !== undefined
      })
    }).catch(() => {});

    const updated = await db.getGrokKeyById(id);
    return c.json({
      id,
      label: updated?.label ?? updates.label ?? existing.label,
      maskedKey: updated?.keyMasked ?? updates.keyMasked ?? existing.keyMasked ?? null,
      status: updated?.status ?? updates.status ?? existing.status,
      cooldownUntil: updated?.cooldownUntil ?? updates.cooldownUntil ?? existing.cooldownUntil
    });
  } catch (error) {
    await db.createAuditLog({
      eventType: 'grok_key.update',
    ...
    return c.json({ error: 'Failed to update Grok key' }, 500);
  }
});

adminRouter.delete('/grok-keys/:id', async (c) => {
  const id = c.req.param('id');
  const ip = c.req.header('CF-Connecting-IP') ?? c.req.header('X-Forwarded-For') ?? null;
  const userAgent = c.req.header('User-Agent') ?? null;
  const db = new D1Client(c.env.DB);
  ...
  try {
    ...
    await db.createAuditLog({
      ...
      ip,
      userAgent,
      detailsJson: JSON.stringify({ label: existing.label })
    }).catch(() => {});
    ...
    return c.json({ error: 'Failed to delete Grok key' }, 500);
  }
});

adminRouter.get('/grok-keys/:id/reveal', async (c) => {
  const id = c.req.param('id');
  const ip = c.req.header('CF-Connecting-IP') ?? c.req.header('X-Forwarded-For') ?? null;
  const userAgent = c.req.header('User-Agent') ?? null;
  const db = new D1Client(c.env.DB);
  ...
  const key = await db.getGrokKeyById(id);
  if (!key) {
    await db.createAuditLog({
      eventType: 'grok_key.reveal',
      outcome: 'not_found',
    ...
    return c.json({ error: 'Grok key not found' }, 404, { 'Cache-Control': 'no-store' });
  }
  ...
  try {
    ...
    await db.createAuditLog({
      ...
      ip,
      userAgent,
      detailsJson: JSON.stringify({ label: key.label })
    }).catch(() => {});
    ...
    return c.json({ error: 'Failed to reveal key' }, 500, { 'Cache-Control': 'no-store' });
  }
});
...
adminRouter.get('/metrics', async (c) => {
  ...
  const [
    ...
    c.env.DB.prepare('SELECT COUNT(*) as count FROM GrokToolUsage WHERE timestamp >= ?').bind(oneMinuteAgo).first<{ count: number }>(),
    ...
    c.env.DB.prepare('SELECT COUNT(*) as count FROM GrokToolUsage WHERE timestamp >= ?').bind(oneHourAgo).first<{ count: number }>(),
    c.env.DB.prepare(`
      SELECT id, toolName, errorMessage, timestamp
      FROM (
        SELECT id, toolName, errorMessage, timestamp FROM TavilyToolUsage WHERE outcome = 'error'
        UNION ALL
        SELECT id, toolName, errorMessage, timestamp FROM BraveToolUsage WHERE outcome = 'error'
        UNION ALL
        SELECT id, toolName, errorMessage, timestamp FROM GrokToolUsage WHERE outcome = 'error'
      ) AS merged
      WHERE timestamp >= ?
  ...
  const activeKeys = tavilyKeys.filter((k: TavilyKey) => k.status === 'active').length +
                     ...
                     grokKeys.filter((k: GrokKey) => k.status === 'active').length;
  const unhealthyKeys = tavilyKeys.filter((k: TavilyKey) => k.status === 'invalid' || k.status === 'cooldown').length +
                        ...
                        grokKeys.filter((k: GrokKey) => k.status === 'invalid' || k.status === 'cooldown').length;
  const activeTokens = clientTokens.filter((t: ClientToken) => !t.revokedAt).length;

  return c.json({
    requestsPerMinute: (tavilyRecentResult?.count ?? 0) + (braveRecentResult?.count ?? 0) + (grokRecentResult?.count ?? 0),
    requestsPerHour: (tavilyHourlyResult?.count ?? 0) + (braveHourlyResult?.count ?? 0) + (grokHourlyResult?.count ?? 0),
    activeTokens,
    keyPool: {
      total: tavilyKeys.length + braveKeys.length + grokKeys.length,
      active: activeKeys,
      ...
      grok: {
        total: grokKeys.length,
        active: grokKeys.filter((k: GrokKey) => k.status === 'active').length,
        cooldown: grokKeys.filter((k: GrokKey) => k.status === 'cooldown').length,
        invalid: grokKeys.filter((k: GrokKey) => k.status === 'invalid').length
      }
    },
