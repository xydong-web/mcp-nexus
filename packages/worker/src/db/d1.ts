...
export interface GrokToolUsage {
  id: string;
  timestamp: string;
  toolName: string;
  outcome: string;
...
export type CombinedUsageLog = TavilyToolUsage | BraveToolUsage | GrokToolUsage;
...
export class D1Client {
  ...
  async getGrokKeys(): Promise<GrokKey[]> {
    const result = await this.db.prepare(`
      SELECT id, label, keyEncrypted, keyMasked, status, cooldownUntil,
             lastUsedAt, failureScore, createdAt, updatedAt
      FROM GrokKey
      ORDER BY createdAt DESC
    `).all<GrokKey>();
    return result.results;
  ...
  async getActiveGrokKey(): Promise<GrokKey | null> {
    const now = new Date().toISOString();
    const result = await this.db.prepare(`
      SELECT * FROM GrokKey
      WHERE status = 'active'
      ...
      ORDER BY lastUsedAt ASC NULLS FIRST
      LIMIT 1
    `).bind(now).first<GrokKey>();
    return result;
  }

  async getGrokKeyById(id: string): Promise<GrokKey | null> {
    const result = await this.db.prepare(`
      SELECT * FROM GrokKey WHERE id = ?
    `).bind(id).first<GrokKey>();
    return result;
  }

  async createGrokKey(data: {
    id: string;
    label: string;
    keyEncrypted: ArrayBuffer;
    keyMasked: string;
    ...
    await this.db.prepare(`
      INSERT INTO GrokKey (id, label, keyEncrypted, keyMasked, status, cooldownUntil, failureScore, createdAt, updatedAt)
      VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?)
    `).bind(data.id, data.label, data.keyEncrypted, data.keyMasked, status, cooldownUntil, now, now).run();
  }

  async updateGrokKey(id: string, data: Partial<GrokKey>): Promise<void> {
    const updates: string[] = [];
    const values: unknown[] = [];
    ...
    if (data.keyEncrypted !== undefined) {
      updates.push('keyEncrypted = ?');
      values.push(data.keyEncrypted);
    }
    if (data.keyMasked !== undefined) {
      updates.push('keyMasked = ?');
      values.push(data.keyMasked);
    }
    if (data.status !== undefined) {
      updates.push('status = ?');
      values.push(data.status);
    ...
    await this.db.prepare(`
      UPDATE GrokKey SET ${updates.join(', ')} WHERE id = ?
    `).bind(...values).run();
  }

  async deleteGrokKey(id: string): Promise<void> {
    await this.db.prepare(`DELETE FROM GrokKey WHERE id = ?`).bind(id).run();
  }
  ...
  async getGrokUsageLogs(limit = 100, offset = 0): Promise<GrokToolUsage[]> {
    const result = await this.db.prepare(`
      SELECT id, timestamp, toolName, outcome, latencyMs,
             clientTokenId, clientTokenPrefix, upstreamKeyId,
             queryHash, queryPreview, argsJson, errorMessage
      ...
      ORDER BY timestamp DESC
      LIMIT ? OFFSET ?
    `).bind(limit, offset).all<GrokToolUsage>();
    return result.results;
  }

  async createGrokToolUsage(data: {
    toolName: string;
    ...
    queryHash?: string | null;
    queryPreview?: string | null;
    argsJson?: Record<string, unknown> | string;
    errorMessage?: string | null;
  }): Promise<void> {
    const id = generateId();
    const timestamp = new Date().toISOString();
    const argsJson = typeof data.argsJson === 'string'
      ? data.argsJson
      : JSON.stringify(data.argsJson ?? {});

    await this.db.prepare(`
      INSERT INTO GrokToolUsage (
        id, timestamp, toolName, outcome, latencyMs,
        clientTokenId, clientTokenPrefix, upstreamKeyId,
        queryHash, queryPreview, argsJson, errorMessage
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      ...
      data.toolName,
      data.outcome,
      typeof data.latencyMs === 'number' ? Math.max(0, Math.floor(data.latencyMs)) : null,
      data.clientTokenId,
      data.clientTokenPrefix ?? null,
      data.upstreamKeyId ?? null,
      data.queryHash ?? null,
  ...
  async getCombinedUsageLogs(
    ...
    const logsResult = await this.db.prepare(`
      ...
      LIMIT ? OFFSET ?
          `).bind(...params, ...params, ...params, limit, offset).all<CombinedUsageLog>();
    ...
    const totalResult = await this.db.prepare(`
      SELECT
        (
          ...
          +
          (SELECT COUNT(*) FROM GrokToolUsage ${clause})
        ) AS totalItems
    `).bind(...params, ...params, ...params).first<{ totalItems: number | string | null }>();
  ...
  async getCombinedUsageSummary(filters: UsageQueryFilters = {}): Promise<UsageSummaryResult> {
    ...
    const byToolResult = await this.db.prepare(`
      ...
      FROM (
        ...
        UNION ALL
        SELECT toolName FROM GrokToolUsage ${clause}
      ) AS merged
      GROUP BY toolName
      ORDER BY count DESC
    `).bind(...params, ...params, ...params).all<{ toolName: string; count: number | string }>();
    ...
    const topQueriesResult = await this.db.prepare(`
      ...
      FROM (
        ...
        UNION ALL
        SELECT queryHash, queryPreview FROM GrokToolUsage ${clause}
      ) AS merged
      WHERE queryHash IS NOT NULL
      GROUP BY queryHash
      ORDER BY count DESC
      LIMIT 20
    `).bind(...params, ...params, ...params).all<{ queryHash: string | null; queryPreview: string | null; count: number | string }>();
