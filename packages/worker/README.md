...
One-click deployment of MCP Nexus to Cloudflare's free tier. This provides a unified MCP server for Tavily, Brave Search, and GrokSearch APIs with automatic API key rotation.
...
- **11 MCP tools**: tavily_search, tavily_extract, tavily_crawl, tavily_map, tavily_research, brave_web_search, brave_local_search, web_search, get_sources, web_fetch, web_map
...
### 3. Add Grok API Keys (Optional, required for Grok tools)

```bash
curl -X POST https://your-worker.workers.dev/admin/api/grok-keys \
  -H "Authorization: Bearer YOUR_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"label": "Primary Grok Key", "key": "xai-xxxxx"}'
```
...
| `GET /admin/api/grok-keys` | Admin Token | List Grok keys |
| `POST /admin/api/grok-keys` | Admin Token | Add Grok key |
| `PATCH /admin/api/grok-keys/:id` | Admin Token | Update Grok key status/metadata |
| `DELETE /admin/api/grok-keys/:id` | Admin Token | Delete Grok key |
| `GET /admin/api/grok-keys/:id/reveal` | Admin Token | Reveal Grok key (rate limited, no-store) |
