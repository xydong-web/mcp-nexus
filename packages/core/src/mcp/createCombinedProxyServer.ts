...
import type { GrokSearchClient } from '../grok/types.js';
import { grokToolsV0100 } from '../grok/tools-v0100.js';
...
type CreateCombinedProxyServerOptions = {
  ...
  getGrokSourceMode?: SearchSourceModeProvider;
  getAllowedTools?: (ctx: unknown) => unknown | Promise<unknown>;  // Phase 3.4: Tool scoping
  getResearchEnabled?: () => boolean | Promise<boolean>;
  getGrokSearchEnabled?: () => boolean | Promise<boolean>;
  getGrokModelDefault?: () => string | Promise<string>;
  getGrokExtraSourcesDefault?: () => number | Promise<number>;
};
...
export function createCombinedProxyServer({
  ...
  getResearchEnabled,
  getGrokSearchEnabled,
  getGrokModelDefault,
  getGrokExtraSourcesDefault
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    ...
    const grokSearchEnabled = await getGrokSearchEnabled?.() ?? false;
    const tavilyTools = researchEnabled
      ? tavilyToolsV0216
      : tavilyToolsV0216.filter(t => t.name !== 'tavily_research');
    const grokTools = grokSearchEnabled && grokClient ? grokToolsV0100 : [];
    return { tools: [...tavilyTools, ...braveToolsV0100, ...grokTools] };
  });
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    ...
    try {
      switch (toolName) {
        ...
        case 'web_search': {
          const enabled = await getGrokSearchEnabled?.() ?? false;
          if (!enabled || !grokClient) {
            return toolError('GrokSearch is disabled by the server administrator.');
          }
          const sourceMode =
          ...
          const response = await grokClient.webSearch(args as any, {
            ...
            modelDefault,
            extraSourcesDefault,
            defaults: getDefaultParameters?.(extra) ?? {}
          });
        ...
        case 'get_sources': {
          ...
          const response = await grokClient.getSources(args as any);
          return textResult(JSON.stringify(response, null, 2));
        }
        case 'web_fetch': {
          const enabled = await getGrokSearchEnabled?.() ?? false;
          ...
          const response = await grokClient.webFetch(args as any);
          return textResult(JSON.stringify(response, null, 2));
        }
        case 'web_map': {
          const enabled = await getGrokSearchEnabled?.() ?? false;
          ...
          const response = await grokClient.webMap(args as any);
          return textResult(JSON.stringify(response, null, 2));
        }
        default:
      ...
      if (isStatusError(error)) {
        const retryHint = typeof error.retryAfterMs === 'number'
          ? ` (retryAfterMs=${Math.max(0, Math.floor(error.retryAfterMs))})`
          : '';
        return toolError(`Grok API error: ${error.message}${retryHint}`);
      }
      throw error;
    }
  });
...
async function handleCombinedWebSearch(
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    if (i < tavilyData.length) {
      ...
      const dedupeUrl = canonicalizeUrl(url);
      if (url && !seenUrls.has(dedupeUrl)) {
...
function normalizeExtraSources(value: number): number {
...
function isStatusError(error: unknown): error is { status: number; message: string; retryAfterMs?: number } {
  return (
    typeof error === 'object' &&
    error !== null &&
