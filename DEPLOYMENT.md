...
# Add Grok API key (optional, required for Grok tools)
curl -X POST https://mcp.yourdomain.com/admin/api/grok-keys \
  -H "Authorization: Bearer YOUR_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
...
curl -X POST https://mcp.yourdomain.com/admin/api/tokens \
  ...
  -d '{
    ...
    "allowedTools": ["tavily_search", "brave_web_search", "web_search", "get_sources", "web_fetch", "web_map"],
    "rateLimit": 120
  }'
```
...
# Expected: {"ok":true,"activeKeys":...,"activeBraveKeys":...,"activeGrokKeys":...}
```
...
### Roll back Grok quickly

If Grok provider quality degrades, disable Grok without impacting existing Tavily/Brave traffic:
